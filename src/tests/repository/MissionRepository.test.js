...
describe("MissionRepository", () => {
  ...
  describe("findById", () => {
    it("deve devolver a missão encontrada", async() => {
      const missao = { _id: MISSION_ID, title: "Explorador" };
    ...
    it("deve popular a turma e quem criou", async() => {
  ...
  describe("findByTitle", () => {
    ...
    it("deve excluir a própria missão da busca", async() => {
      ...
      await repository.findByTitle("Explorador", MISSION_ID);
  ...
  describe("list", () => {
    ...
    it("deve montar os filtros pelo MissionFilterBuild", async() => {
      ...
      await repository.list({
        query: { title: "explor", type: "quiz", class_id: CLASS_ID, active: "true" },
      });
    ...
    it("deve respeitar página e limite informados", async() => {
      ...
      await repository.list({ query: { page: "2", limit: "15" } });

      const [, opcoes] = modelo.paginate.mock.calls[0];
      expect(opcoes.page).toBe(2);
      expect(opcoes.limit).toBe(15);
    });
  });

  describe("create", () => {
    it("deve instanciar o model com os dados e salvar", async() => {
      const salva = { _id: MISSION_ID, title: "Explorador" };
      const save = jest.fn().mockResolvedValue(salva);
      modelo.mockImplementation(() => ({ save }));

      const resultado = await repository.create({ title: "Explorador" });

      expect(modelo).toHaveBeenCalledWith({ title: "Explorador" });
  ...
  describe("update", () => {
    it("deve devolver a missão já atualizada", async() => {
      ...
      const resultado = await repository.update(MISSION_ID, { xp_reward: 200 });

      expect(modelo.findByIdAndUpdate).toHaveBeenCalledWith(MISSION_ID, { xp_reward: 200 }, { new: true });
    ...
    it("deve lançar 404 quando a missão não existir", async() => {
      ...
      const erro = await capturarErro(repository.update(MISSION_ID, { xp_reward: 200 }));
